...
public class Ex01_2 extends JFrame {
	public Ex01_2(){
		setTitle("FlowLayout");
		// 원래 기본적으로 BorderLayout인 JFrame을 FlowLayout으로 변경
//		setLayout(new FlowLayout());
		
		// JPanel을 배치 관리자(FlowLayout)를 이용해 왼쪽 정렬과 오른쪽 정렬을 해 보자.
...
//		JPanel jp = new JPanel();
		...
		add(jb1,BorderLayout.EAST);		// 상수를 호출하는 것이므로 전체 대문자
		add(jb2,BorderLayout.WEST);
		add(jb3,BorderLayout.SOUTH);
		add("North",jb4);				// String으로 이름을 불러 상수를 호출
		add("Center",jb5);
//		add(jb6);
